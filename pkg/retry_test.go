...
import (
	...
	"github.com/rnymphaea/chronoflow/pkg"
)
...
func TestRetryDelay(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := pkg.RetryDelay(cfg, tt.attempt)
...
func TestRetryDelay_CoversNegativeDelayBranch(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		delay := pkg.RetryDelay(cfg, attempt)
		if delay <= 0 {
...
func BenchmarkRetryDelay(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_ = pkg.RetryDelay(cfg, i%cfg.MaxAttempts)
	}
}
